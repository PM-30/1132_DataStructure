...
private:
    ...
    TreeNode* deleteNode(TreeNode* node, int val) {
        if (node == nullptr) return nullptr; // 找不到就回傳空

        if (val < node->data) {
            node->left = deleteNode(node->left, val); // 往左找
        }
        else if (val > node->data) {
            node->right = deleteNode(node->right, val); // 往右找
        }
        else {
            // 找到了！
            if (node->left == nullptr && node->right == nullptr) {//沒有小孩
                delete node;//直接刪除
                return nullptr;//回傳空的
            }
            else if (node->left == nullptr) {// 只有右小孩
                TreeNode* temp = node->right;//右小孩補上
                delete node;
                return temp;//回傳右小孩
            }
            else if (node->right == nullptr) {//只有左小孩
                TreeNode* temp = node->left;//左小孩補上
                delete node;
                return temp;//回傳左小孩
            }
            else {//左右小孩都有
                TreeNode* temp = findMin(node->right);//從右子樹找最小的取代
                node->data = temp->data;//將數值替代回去
                node->right = deleteNode(node->right, temp->data); // 刪右邊那個最小值
            }
        }
        return node;// 回傳處理完後的節點讓上一層能更新子樹
    }
